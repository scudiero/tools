...
## Import thins we need to continue
	...
	includes="$includes CheckRun CheckAuth GetDefaultsData Call ParseArgs ParseArgsStd Hello Init Goodbye SendEmail"
	Import "$includes"
	prtStatus ", imports"
	sTime=$(date "+%s")
...
## XO NOT AUTOVERSION
#===================================================================================================
version="1.3.47" # -- dscudiero -- Thu 08/24/2017 @ 10:34:23.06
#===================================================================================================
# $callPgmName "$executeFile" ${executeFile##*.} "$libs" $scriptArgs
#===================================================================================================
# Copyright 2016 David Scudiero -- all rights reserved.
...
## Import thins we need to continue
	Import 'Msg2 PushSettings SetFileExpansion Upper PopSettings TrapSigs PadChar GetDefaultsData RunSql2'
...
## Import generally used library functions
	sTime=$(date "+%s")
		sqlStmt="select scriptData1 from $scriptsTable where name=\"$myName\""
		RunSql2 $sqlStmt
		unset includes
		if [[ ${#resultSet[@]} -ne 0 ]]; then
			includes="$(tr ',' ' ' <<< "$resultSet[0]")"
		fi
[[ $LOGNAME == 'dscudiero' ]] && echo "includes = '$includes'"
	# includes='StringFunctions Msg2 Dump Here Quit PushSettings PopSettings MkTmpFile Pause ProtectedCall'
	# includes="$includes SetFileExpansion PadChar PrintBanner Alert TrapSigs SignalHandeler RunSql2 Prompt VerifyPromptVal"
	# includes="$includes DbLog ProcessLogger GetCallStack DisplayNews Help Call StartRemoteSession FindExecutable"
	# includes="$includes CheckRun CheckAuth GetDefaultsData Call ParseArgs ParseArgsStd Hello Init Goodbye SendEmail"
...
## XO NOT AUTOVERSION
#===================================================================================================
version="1.3.51" # -- dscudiero -- Thu 08/24/2017 @ 10:43:34.84
#===================================================================================================
# $callPgmName "$executeFile" ${executeFile##*.} "$libs" $scriptArgs
#===================================================================================================
# Copyright 2016 David Scudiero -- all rights reserved.
...
## Import thins we need to continue
	Import 'Msg2 PushSettings SetFileExpansion StringFunctions PopSettings TrapSigs PadChar GetDefaultsData RunSql2'
...
## Import generally used library functions
	...
	RunSql2 $sqlStmt
	unset includes
	[[ ${#resultSet[@]} -ne 0 ]] && Import "$(tr ',' ' ' <<< "$resultSet")"
	prtStatus ", imports"
